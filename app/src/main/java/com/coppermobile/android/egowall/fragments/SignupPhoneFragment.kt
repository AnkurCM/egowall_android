...
class SignupPhoneFragment : BaseFragment() {
    ...
    private fun phoneNumberTextWatcher() {
        ...
        et_signup_phone.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable?) {
                if (s.toString() != "") {

                    if (et_signup_phone.text.toString() != "") {
                        bt_signup_phone_next.isEnabled = true
                        bt_signup_phone_next.background =
                            ContextCompat.getDrawable(activity!!, R.drawable.btn_login_filled)
                        bt_signup_phone_next.setTextColor(ContextCompat.getColor(activity!!, R.color.white))
                    }

                } else {

                    bt_signup_phone_next.isEnabled = false
                    bt_signup_phone_next.background =
                        ContextCompat.getDrawable(activity!!, R.drawable.btn_login_filled)
                    bt_signup_phone_next.setTextColor(ContextCompat.getColor(activity!!, R.color.white))
                }
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                til_signup_phone.error = null

                if (s != null && s.isNotEmpty()) {
                    til_signup_phone.setHintTextAppearance(R.style.LoginTextInputLayoutFilledStyle)
                    til_signup_phone.boxStrokeColor = ContextCompat.getColor(activity!!, R.color.color_479EA1)
                } else {
                    til_signup_phone.setHintTextAppearance(R.style.LoginTextInputLayoutStyle)
                    til_signup_phone.boxStrokeColor = ContextCompat.getColor(activity!!, R.color.black)
                }
    ...
    private fun clickListener() {
        ...
        bt_signup_phone_next.setOnClickListener {
    ...
    private fun validateData(): Boolean {
        ...
        val phoneNumber = et_signup_phone.text.toString()
        ...
        if (Helpers.isPhoneValid(phoneNumber)) {
            ...
            if (phoneNumber == "" || !Helpers.isPhoneValid(phoneNumber)) {
                til_signup_phone.error = getString(R.string.check_your_phone_number)
            }
        }
        return false
